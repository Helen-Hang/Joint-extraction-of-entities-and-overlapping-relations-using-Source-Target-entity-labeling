...
class Config:
    def __init__(self, configs):
        home = '/home/wyf/desktop/multi_re'
        ...
        model_home = os.path.join(home, 'data', DataSet.name, 'models')
        if self.train_method == TrainMethod.NLL_METHOD:
            runner = '%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s' % (
            self.exp_name, self.dataset_name, self.decoder_method, self.train_method,
