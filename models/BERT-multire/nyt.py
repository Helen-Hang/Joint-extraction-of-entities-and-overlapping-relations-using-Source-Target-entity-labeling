...
# 加载数据集
train_data = load_data('./raw_data/nyt/train_data.json')
valid_data = load_data('./raw_data/nyt/dev_data.json')
test_data = load_data('./raw_data/nyt/test_data.json')
predicate2id, id2predicate = {}, {}


with codecs.open('./raw_data/nyt/all_24_schemas.json', encoding='utf-8') as f:
    for l in f:
        l = json.loads(l)
...
# # 预测subject
output = LayerNormalization()(bert.model.output)
...
def extract_spoes(text):
    ...
    if subjects:
        ...
        for subject, object_pred in zip(subjects, object_preds):
            ...
            end = np.where(object_pred[:, :, 1] > 0.6)
...
def evaluate(data):
    ...
    for d in data:
        ...
        pbar.update()
        pbar.set_description('f1: %.5f, precision: %.5f, recall: %.5f, acc: %.5f' %(f1, precision, recall, acc))
        #对数据进行编码
        s = json.dumps(
            {
                'sentText': d['sentText'],
...
if __name__ == '__main__':
    ...
    for i in range(5001, 56195, 5000):
    ...
    plt.plot([i for i in range(5001, 56195, 5000)], np.sqrt(train_mse), label='train')
    plt.plot([i for i in range(5001, 56195, 5000)], np.sqrt(test_mse), label='test')
    plt.xlabel('Number of training samples', fontsize=16)
    plt.ylabel('RMSE', fontsize=16)
    plt.title(r'Learning curve on the NYT dataset', fontsize=16)
    plt.legend()
    plt.savefig("learning curves1.png")
    plt.show()

    plt.plot([i for i in range(5001, 56195, 5000)], train_acc_sum, label='train')
    plt.plot([i for i in range(5001, 56195, 5000)], test_acc_sum, label='test')
    plt.xlabel('Number of training samples', fontsize=16)
    ...
    plt.plot([i for i in range(5001, 56195, 5000)], train_acc_ave, label='train')
    plt.plot([i for i in range(5001, 56195, 5000)], test_acc_ave, label='test')
    plt.xlabel('Number of training samples', fontsize=16)
    ...
    # train_model.load_weights('best_model-0.5.weights')
    # print(u'final test f1: %.5f, precision: %.5f, recall: %.5f\n' % (evaluate(test_data)))
    # print(u'final train f1: %.5f, precision: %.5f, recall: %.5f\n' % (evaluate(train_data)))
