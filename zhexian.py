...
plt.plot(x, y, marker='o', color='g', label=u'Precision')
plt.plot(x, y1, marker='*', color='b',label=u'Recall')
plt.plot(x, y2, marker='s', color='r',label=u'F1')
...
plt.title("Results on different relation threshold(NYT)", fontsize=15) #标题
plt.savefig("./fig12-1.eps",dpi=600)
plt.show()
...
# plt.xlabel(u"Relation threshold", fontsize=15) #X轴标签
# plt.ylabel("Value of (P,R,F1)", fontsize=15) #Y轴标签
# plt.title("Results on different relation threshold(WebNLG)", fontsize=15) #标题
# plt.savefig("./fig12-2.eps",dpi=600)
# plt.show()
